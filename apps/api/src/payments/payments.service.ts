...
export class PaymentsService {
  ...
  async createPaymentIntent(
    ...
    try {
      ...
      if (!paymentPlugins || paymentPlugins.length === 0) {
        throw new BadRequestException('No payment provider is configured for this organization');
      }

      // Use the first enabled payment plugin (in the future, could allow selection)
      const paymentPlugin = paymentPlugins[0];
      const providerName = paymentPlugin.plugin.metadata?.paymentProvider || 'unknown';
      ...
      if (!response.success) {
        throw new BadRequestException(`Failed to create payment intent: ${response.error}`);
      }
  ...
  async confirmPayment(
    ...
    try {
      ...
      if (!response.success) {
        throw new BadRequestException(`Failed to confirm payment: ${response.error}`);
      }
  ...
  async processRefund(
    ...
    try {
      ...
      if (!response.success) {
        throw new BadRequestException(`Failed to process refund: ${response.error}`);
      }
  ...
  private async makePluginRequest(
    ...
    try {
      ...
      return {
        success: false,
        error: error.message || 'Unknown error occurred when calling payment plugin',
      };
    }
  }
  ...
  async createTransaction(
    ...
    try {
      ...
      let gatewayFee = amount * 0.029 + 0.30; // Default: 2.9% + $0.30
