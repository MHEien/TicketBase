...
export class Transaction {
  ...
  @ApiProperty({ description: 'Transaction type', enum: TransactionType, enumName: 'TransactionType' })
  @Column({
    type: 'enum',
  ...
  @ApiProperty({ description: 'Payment method ID from provider', example: 'pm_1234567890' })
  @Column({ name: 'payment_method_id' })
  paymentMethodId: string;

  @ApiProperty({ description: 'Payment intent ID from provider', required: false, example: 'pi_1234567890' })
  @Column({ name: 'payment_intent_id', nullable: true })
  paymentIntentId: string;

  @ApiProperty({ description: 'Transaction status', enum: TransactionStatus, enumName: 'TransactionStatus' })
  @Column({
    type: 'enum',
  ...
  @ApiProperty({ description: 'Error message (if transaction failed)', required: false })
