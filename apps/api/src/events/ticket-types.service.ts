...
export class TicketTypesService {
  ...
  async create(eventId: string, organizationId: string, createTicketTypeDto: CreateTicketTypeDto): Promise<TicketType> {
  ...
  async findOne(id: string, eventId: string, organizationId: string): Promise<TicketType> {
    // Verify event belongs to organization
    const event = await this.eventsRepository.findOne({
      where: { id: eventId, organizationId },
    });
  ...
  async update(id: string, eventId: string, organizationId: string, updateTicketTypeDto: UpdateTicketTypeDto): Promise<TicketType> {
    ...
    if (updateTicketTypeDto.quantity !== undefined) {
      ...
      if (newQuantity < soldTickets) {
        throw new BadRequestException(`Cannot reduce quantity below sold tickets count (${soldTickets})`);
      }
  ...
  async remove(id: string, eventId: string, organizationId: string): Promise<void> {
    // Fetch the ticket type ensuring it belongs to the organization's event
    const ticketType = await this.findOne(id, eventId, organizationId);
