...
export class EventsService {
  ...
  async create(organizationId: string, userId: string, createEventDto: CreateEventDto): Promise<Event> {
    ...
    if (createEventDto.ticketTypes && createEventDto.ticketTypes.length > 0) {
      ...
      const savedTicketTypes = await this.ticketTypesRepository.save(ticketTypes);
  ...
  async findAll(organizationId: string, options?: {
    ...
    if (options?.category) {
      queryBuilder.andWhere('event.category = :category', { category: options.category });
    }
    ...
    if (options?.startDate) {
      queryBuilder.andWhere('event.startDate >= :startDate', { startDate: options.startDate });
    }
    
    if (options?.endDate) {
      queryBuilder.andWhere('event.endDate <= :endDate', { endDate: options.endDate });
    }
  ...
  async update(id: string, organizationId: string, userId: string, updateEventDto: UpdateEventDto): Promise<Event> {
  ...
  async cancel(id: string, organizationId: string, userId: string): Promise<Event> {
    const event = await this.findOne(id, organizationId);
