import { Injectable, Logger, NotFoundException, StreamableFile, InternalServerErrorException } from '@nestjs/common';
...
export class BundleService {
  ...
  constructor(
    ...
    this.minioClient = new Minio.Client({
      endPoint: this.configService.get<string>('plugins.minioEndpoint', 'localhost'),
  ...
  async getPluginBundle(pluginId: string): Promise<{
    ...
    const minioBucket = this.configService.get<string>('plugins.minioBucket', 'plugins');
    const minioObjectName = `${plugin.category}/${fileName}`;
    
    try {
      // Check if the plugin bundle exists in MinIO
      const stat = await this.minioClient.statObject(minioBucket, minioObjectName)
        .catch(() => null);
      
      if (stat) {
        this.logger.log(`Fetching plugin bundle from MinIO: ${minioObjectName}`);
        
        // Get the object stream from MinIO
        const stream = await this.minioClient.getObject(minioBucket, minioObjectName);
    ...
    if (plugin.bundleUrl.startsWith('http')) {
      try {
        this.logger.log(`Fetching remote plugin bundle from URL: ${plugin.bundleUrl}`);
        ...
        this.logger.error(`Failed to fetch plugin bundle from URL: ${error.message}`);
        throw new NotFoundException(`Plugin bundle for ${plugin.name} could not be retrieved`);
      }
    }
    
    // If we get here, the bundle wasn't found
    throw new NotFoundException(`Plugin bundle for ${plugin.name} not found in storage or at URL`);
  }
} 