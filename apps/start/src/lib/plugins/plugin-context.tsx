...
interface PluginContextValue {
  ...
  // Plugin management
  loadPlugin: (pluginId: string, extensionPoint: string) => Promise<ExtensionPointComponent>;
...
export function PluginProvider({ children }: PluginProviderProps) {
  ...
  const pluginComponents = React.useRef<Map<string, ExtensionPointComponent>>(new Map());
  ...
  const loadPlugin = React.useCallback(async (pluginId: string, extensionPoint: string) => {
    ...
    if (!plugin.extensionPoints.includes(extensionPoint)) {
      throw new Error(`Plugin ${pluginId} does not support extension point ${extensionPoint}`);
    }
    ...
    if (!component) {
      throw new Error(`Failed to load plugin component for ${pluginId} ${extensionPoint}`);
    }
  ...
  const getExtensionPoint = React.useCallback((extensionPoint: string): ExtensionPointComponent[] => {
    return plugins
      .filter(p => p.enabled && p.extensionPoints.includes(extensionPoint))
      .map(p => {
      ...
      .filter((component): component is ExtensionPointComponent => component !== undefined);
  }, [plugins]);
