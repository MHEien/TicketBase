...
export class PluginStorageService {
  ...
  constructor(
    ...
    this.bucketName = this.configService.get('PLUGIN_ASSETS_BUCKET', 'plugin-bundles');
    this.serverUrl = this.configService.get('PLUGIN_SERVER_URL', 'http://localhost:4000');

    // Ensure bucket exists
    this.initBucket().catch((err) => {
      this.logger.error(`Failed to initialize Minio bucket: ${err.message}`, err);
    });
  }

  private async initBucket() {
    try {
      const exists = await this.minioService.client.bucketExists(this.bucketName);
      if (!exists) {
  ...
  async storePluginBundle(
    ...
    try {
      ...
      this.logger.log(`Stored plugin bundle for ${pluginId} v${version} at ${bundleUrl}`);
      return bundleUrl;
    } catch (error) {
      this.logger.error(
  ...
  async getPluginBundleStream(objectKey: string) {
    try {
      return await this.minioService.client.getObject(this.bucketName, objectKey);
    } catch (error) {
  ...
  async deletePluginBundle(pluginId: string, version?: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to delete plugin bundle: ${error.message}`, error);
      throw error;
    }
  }
  ...
  private async listObjects(prefix: string): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const objects: any[] = [];
      const stream = this.minioService.client.listObjectsV2(this.bucketName, prefix, true);
   ...
   */
  async checkStorageConnection(): Promise<{ isConnected: boolean; message?: string }> {
    try {
      ...
      this.logger.error(`MinIO connection check failed: ${error.message}`, error);
      return { 
        isConnected: false, 
