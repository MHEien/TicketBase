...
export class PluginProxyController {
  ...
  async proxyRequest(
    ...
    this.logger.debug(`🔑 Headers: ${JSON.stringify(this.sanitizeHeaders(req.headers))}`);
    ...
    try {
      ...
      this.logger.debug(`🔍 Checking if plugin is installed for tenant: ${tenantId}`);
      ...
      if (!isInstalled) {
        this.logger.warn(`❌ Plugin ${pluginId} not installed for tenant ${tenantId}`);
        return res.status(404).json({ error: `Plugin ${pluginId} not installed` });
      }
      
      this.logger.debug(`✅ Plugin ${pluginId} is installed for tenant ${tenantId}`);
      ...
      try {
        ...
        this.logger.debug(`📥 Received response from plugin service: ${response.status}`);
        ...
        if (error.response) {
          this.logger.error(`❌ Plugin service error: ${error.response.status} - ${JSON.stringify(error.response.data)}`);
          return res
