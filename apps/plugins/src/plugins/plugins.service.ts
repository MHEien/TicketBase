...
export class PluginsService {
  ...
  async installPlugin(tenantId: string, pluginId: string): Promise<any> {
    ...
    if (existing) {
      throw new BadRequestException(`Plugin ${pluginId} is already installed for tenant ${tenantId}`);
    }
  ...
  async createPlugin(
    ...
    if (!finalBundleUrl) {
      try {
        ...
        const storageStatus = await this.pluginStorageService.checkStorageConnection();
        if (!storageStatus.isConnected) {
          throw new Error(storageStatus.message);
        }

        // Generate the bundle from source
        const bundleBuffer = await this.bundleService.generateBundleBuffer(id, sourceCode);
        ...
        this.logger.error(`Failed to store plugin bundle: ${error.message}`, error);
  ...
  async updatePlugin(
    ...
    if (updates.requiredPermissions) updateData.requiredPermissions = updates.requiredPermissions;
    ...
    if (updates.sourceCode) {
      ...
      // Validate plugin structure
      const isValid = await this.bundleService.validatePluginStructure(updates.sourceCode);
      if (!isValid) {
        throw new BadRequestException('Invalid plugin structure');
      }
      
      // Analyze plugin bundle to extract metadata and extension points
      const { extensionPoints, metadata } = await this.bundleService.analyzeBundle(updates.sourceCode);
      
      // Version is required for storage
      const version = updates.version || plugin.version;
      
      // Generate the bundle from source
      const bundleBuffer = await this.bundleService.generateBundleBuffer(id, updates.sourceCode);
  ...
  async getPluginsByExtensionPoint(extensionPoint: string): Promise<PluginDocument[]> {
   ...
   */
  async checkStorageHealth(): Promise<{ isConnected: boolean; message?: string }> {
