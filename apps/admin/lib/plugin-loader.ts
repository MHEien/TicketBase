  ...
  try {
    ...
    if (window.__PLUGIN_REGISTRY?.registered[plugin.id]) {
      ...
      loadedModules[plugin.id] = { 
        ...
        extensionPoints: window.__PLUGIN_REGISTRY.registered[plugin.id].extensionPoints
      };
      return loadedModules[plugin.id];
    }
    ...
    const importFn = new Function('url', 'return import(/* webpackIgnore: true */ url)');
 ...
 */
export async function loadPluginComponent(plugin: InstalledPlugin, componentPath: string): Promise<React.ComponentType<any> | null> {
  try {
    ...
    if (!component || typeof component !== 'function') {
      console.error(`Component ${componentPath} not found in plugin ${plugin.id} or is not a valid component`);
      return null;
    }
    
    return component;
  } catch (error) {
    console.error(`Failed to load component ${componentPath} from plugin ${plugin.id}:`, error);
    return null;
  }
}
 ...
 */
export async function loadExtensionPointComponent(plugin: InstalledPlugin, extensionPoint: string): Promise<React.ComponentType<any> | null> {
  try {
    // Load the plugin module
    const module = await loadPluginModule(plugin);
    ...
    if (!plugin.extensionPoints?.includes(extensionPoint)) {
      console.warn(`Plugin ${plugin.id} does not implement extension point: ${extensionPoint}`);
      return null;
    }
    
    // Get the extension point component
    if (!module.extensionPoints || !module.extensionPoints[extensionPoint]) {
      console.error(`Extension point ${extensionPoint} not found in plugin ${plugin.id}`);
      return null;
    }
    ...
    if (typeof component !== 'function') {
      console.error(`Extension point ${extensionPoint} in plugin ${plugin.id} is not a valid component`);
      return null;
    }
    
    return component;
  } catch (error) {
    console.error(`Failed to load extension point ${extensionPoint} from plugin ${plugin.id}:`, error);
    return null;
  }
} 