...
function loadPluginScript(plugin: InstalledPlugin): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = (err) => reject(new Error(`Failed to load plugin script: ${plugin.bundleUrl}`));
 ...
 */
export async function registerDevPlugin(plugin: InstalledPlugin): Promise<void> {
