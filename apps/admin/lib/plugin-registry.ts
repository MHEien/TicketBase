...
class PluginRegistry {
  ...
  getPluginsByCategory(category: Plugin['category']): InstalledPlugin[] {
    return this.plugins.filter(plugin => plugin.category === category && plugin.enabled);
  }

  /**
   * Get all plugins that have a specific component type
   */
  getPluginsWithComponent(componentType: 'settings' | 'eventCreation' | 'dashboard' | 'checkout' | 'eventDetail' | 'ticketSelection'): InstalledPlugin[] {
    return this.plugins.filter(plugin => {
      if (componentType === 'settings' && plugin.adminComponents.settings) return true;
      if (componentType === 'eventCreation' && plugin.adminComponents.eventCreation) return true;
      if (componentType === 'dashboard' && plugin.adminComponents.dashboard) return true;
      if (componentType === 'checkout' && plugin.storefrontComponents.checkout) return true;
      if (componentType === 'eventDetail' && plugin.storefrontComponents.eventDetail) return true;
      if (componentType === 'ticketSelection' && plugin.storefrontComponents.ticketSelection) return true;
      return false;
