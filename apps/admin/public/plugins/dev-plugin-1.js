...
window.devPlugin1 = (function() {
  ...
  const PaymentMethodComponent = function({ context, configuration, plugin }) {
    ...
    return React.createElement('div', { 
      ...
      React.createElement('h3', { key: 'title' }, 'Development Test Payment Method'),
      React.createElement('p', { key: 'desc' }, `This is a test payment method from ${plugin.name}`),
      React.createElement('p', { key: 'amount' }, `Amount: ${context.cart?.total || 0}`),
  ...
  const AdminSettingsComponent = function({ context, configuration, plugin }) {
    return React.createElement('div', null, [
      ...
      React.createElement('p', { key: 'desc' }, `This is the settings panel for ${plugin.name}`),
      React.createElement('p', { key: 'config' }, `Current config: ${JSON.stringify(configuration)}`),
    ]);
  };
