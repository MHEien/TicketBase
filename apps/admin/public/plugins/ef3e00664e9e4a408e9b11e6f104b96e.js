...
try {
  ...
  execSync(
    'npx esbuild index.tsx --bundle --format=esm --outfile=dist/remoteEntry.js --external:react --external:react-dom --target=es2020',
    { stdio: 'inherit', cwd: __dirname }
  );

  // Also create a legacy bundle for compatibility
  execSync(
  ...
  console.log(`New format: ${path.join(distDir, 'remoteEntry.js')}`);
  console.log(`Legacy format: ${path.join(distDir, 'stripe-plugin.js')}`);
