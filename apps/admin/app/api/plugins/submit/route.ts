...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['name', 'description', 'version', 'category', 'bundleUrl', 'authorName', 'authorEmail'];
    ...
    // Validate category
    const validCategories = ['payments', 'marketing', 'analytics', 'social', 'ticketing', 'layout', 'seating'];
    ...
    if (!data.bundleUrl.startsWith('/plugins/') && !data.bundleUrl.startsWith('http')) {
      return NextResponse.json(
        { error: 'Bundle URL must be a valid URL or a path to an uploaded plugin' },
        { status: 400 }
      );
    }
    ...
    const PLUGIN_SERVER_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Failed to submit to plugin server:', await response.text());
