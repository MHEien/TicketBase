...
import { fetchAvailablePlugins, installPlugin, uninstallPlugin } from "@/lib/plugin-api"
...
export function PluginGallery() {
  ...
  const { plugins: installedPlugins, loading: loadingInstalled, error: installedError, refreshPlugins } = usePlugins()
    ...
    async function loadAvailablePlugins() {
      try {
        ...
        if (response.success && response.data) {
          ...
          setAvailableError(new Error(response.error || 'Failed to load plugins'))
        }
      } catch (error) {
        setAvailableError(error instanceof Error ? error : new Error(String(error)))
      } finally {
    ...
    <div className="h-full space-y-6 overflow-y-auto">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={() => window.location.href = "/settings/plugins/submit"} className="gap-2">
          ...
          <Button variant="outline" size="icon" onClick={() => setView(view === "grid" ? "list" : "grid")}>
            {view === "grid" ? <Grid3X3 className="h-4 w-4" /> : <Layers className="h-4 w-4" />}
          </Button>
        </div>
      </div>
                      ...
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-primary/10">
                            {(() => {
                              const plugin = filteredPlugins.find((p) => p.id === selectedPlugin)
                              const PluginIcon = plugin?.icon
                              return PluginIcon ? <PluginIcon className="h-6 w-6 text-primary" /> : null
                            })()}
                          </div>
                          <div>
                            <CardTitle>{filteredPlugins.find((p) => p.id === selectedPlugin)?.name}</CardTitle>
                            <CardDescription>
                              By {filteredPlugins.find((p) => p.id === selectedPlugin)?.developer} •{" "}
                              {filteredPlugins.find((p) => p.id === selectedPlugin)?.installs} installs
                            </CardDescription>
                          </div>
                        </div>
                        <Button onClick={() => setSelectedPlugin(null)} variant="ghost">
                          Back to Gallery
                        </Button>
                      </CardHeader>
                      <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                          <div className="space-y-1 md:col-span-2">
                            ...
                            <p className="text-muted-foreground">
                              {filteredPlugins.find((p) => p.id === selectedPlugin)?.description} Lorem ipsum dolor sit amet,
                              ...
                              return plugin.installed ? (
                                <>
                                  ...
                                  <div className="flex flex-col gap-2">
                                    ...
                                    >
                                      {isUninstalling && actionPlugin === plugin.id ? (
                                        <>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center gap-3">
                            <Avatar>
                              <AvatarImage
                                src={filteredPlugins.find((p) => p.id === selectedPlugin)?.developerAvatar || "/placeholder.svg"}
                              />
                              <AvatarFallback>
                            ...
                            <div>
                              <p className="font-medium">{filteredPlugins.find((p) => p.id === selectedPlugin)?.developer}</p>
                              <p className="text-sm text-muted-foreground">12 plugins • Since 2020</p>
                            </div>
                          </div>
                        </div>
                      </CardContent>
                  ...
                  >
                    ...
                    {filteredPlugins
                      .filter((plugin) => {
                        ...
                        if (category.id === "installed") return plugin.installed;
                            ...
                            <Card className="cursor-pointer transition-all duration-300 hover:border-primary/20 hover:shadow-md">
                              <CardHeader className="pb-2">
                                <div className="flex items-start justify-between">
                                  ...
                                  <Badge variant={plugin.installed ? "outline" : "secondary"}>
