 ...
 * - "event-detail-header"
 */
export function PluginWidgetArea({ areaName, eventData, className = "" }: PluginWidgetAreaProps) {
      ...
      try {
        ...
        for (const plugin of relevantPlugins) {
          ...
          try {
            ...
            console.error(`Failed to load widget from plugin ${plugin.id} for area ${areaName}:`, e);
          }
        }
  ...
  return (
    <div className={`plugin-widget-area ${className}`} data-area-name={areaName}>
      {loading ? (
