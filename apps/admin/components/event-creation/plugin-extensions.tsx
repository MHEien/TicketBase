 ...
 */
export function PluginExtensions({ formData, onChange }: PluginExtensionsProps) {
  ...
  useEffect(() => {
    ...
    async function loadPlugins() {
      try {
        ...
        const eventPlugins = pluginRegistry.getPluginsWithComponent('eventCreation');
        ...
        for (const plugin of eventPlugins) {
          if (!plugin.enabled || !plugin.adminComponents.eventCreation) continue;
          ...
          try {
            ...
            console.error(`Failed to load event creation component for plugin ${plugin.id}:`, e);
          }
        }
