...
interface UseEventsReturn {
  ...
  updateEventMutation: (id: string, data: UpdateEventDto) => Promise<Event | null>;
...
export function useEvents(params?: EventsQueryParams): UseEventsReturn {
  ...
  const createEventMutation = useCallback(async (data: CreateEventDto): Promise<Event | null> => {
    try {
  ...
  const updateEventMutation = useCallback(async (id: string, data: UpdateEventDto): Promise<Event | null> => {
    try {
  ...
  const deleteEventMutation = useCallback(async (id: string): Promise<boolean> => {
    try {
  ...
  const publishEventMutation = useCallback(async (id: string): Promise<Event | null> => {
    try {
      const publishedEvent = await publishEvent(id);
      setEvents(prev => prev.map(event => event.id === id ? publishedEvent : event));
      toast({
        title: "Success",
  ...
  const cancelEventMutation = useCallback(async (id: string): Promise<Event | null> => {
    try {
      const cancelledEvent = await cancelEvent(id);
      setEvents(prev => prev.map(event => event.id === id ? cancelledEvent : event));
      toast({
        title: "Success",
