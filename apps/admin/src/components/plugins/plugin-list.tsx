...
export function PluginList() {
  ...
  const fetchPlugins = async () => {
    try {
      ...
      try {
        const installedResponse = await fetch('/api/plugins/organization/current');
    ...
    return (
      <div className="space-y-4">
        <div className="flex space-x-4 mb-6">
          <Button variant="outline" disabled>Available</Button>
  ...
  const pluginsToShow = activeTab === "available" ? availablePlugins : installedPlugins;
              ...
              <Card key={plugin.id} className="flex flex-col">
                <CardHeader className="flex-1">
                  ...
                  <div className="space-y-2 mt-3">
                    <Badge 
                      variant="secondary" 
                      className={categoryColors[plugin.category as keyof typeof categoryColors] || categoryColors.utility}
                    >
                      {plugin.category}
                    </Badge>
                  ...
                  {plugin.extensionPoints && plugin.extensionPoints.length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs text-muted-foreground mb-1">Extension Points:</p>
