...
class PluginManager {
   ...
   */
  private registerExtensionPoint(extensionPoint: string, plugin: LoadedPlugin): void {
   ...
   */
  private async loadRemotePlugin(bundleUrl: string, pluginId: string): Promise<any> {
    try {
      // Check if Module Federation is available
      const hasMF = typeof window !== 'undefined' && (window as any).__FEDERATION__;
   ...
   */
  private async injectPluginScript(bundleUrl: string, pluginId: string): Promise<void> {
  ...
  renderExtensionPoint(
    ...
    return plugins
      .filter(plugin => plugin.isLoaded && plugin.extensionPoints[extensionPoint])
      .map(plugin => {
