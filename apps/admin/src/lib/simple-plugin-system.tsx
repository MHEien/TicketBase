...
class SimplePluginLoader {
  ...
  async loadPlugin(metadata: PluginMetadata): Promise<LoadedPlugin> {
    ...
    try {
      ...
      console.log(`🔌 Expected extension points: ${metadata.extensionPoints.join(', ')}`);

      const module = await import(/* @vite-ignore */ metadata.bundleUrl);
      console.log(`🔌 Loaded module:`, module);
      console.log(`🔌 Module default:`, module.default);
      console.log(`🔌 Module default extensionPoints:`, module.default?.extensionPoints);
      console.log(`🔌 Module keys:`, Object.keys(module));
      console.log(`🔌 Module default keys:`, module.default ? Object.keys(module.default) : 'no default');
      
      // Try to understand the structure better
      if (module.default) {
        console.log(`🔌 Module default type:`, typeof module.default);
        console.log(`🔌 Module default constructor:`, module.default.constructor?.name);
        console.log(`🔌 Module default stringified:`, JSON.stringify(module.default, null, 2));
      }
      ...
      if (module.default?.extensionPoints) {
        ...
        console.log(`🔌 Found extensionPoints via module.default.extensionPoints`);
      }
      ...
      else if (module.default && typeof module.default === 'object' && metadata.extensionPoints.some(point => module.default[point])) {
        extensionPoints = module.default;
        console.log(`🔌 Found extensionPoints as direct properties on module.default`);
      }
      ...
      if (extensionPoints) {
        for (const point of metadata.extensionPoints) {
          ...
          if (component && typeof component === 'function') {
            ...
            console.log(`✅ Registered component for extension point: ${point}`);
          } else {
            console.log(`❌ No valid component found for extension point: ${point}`);
          }
        }
      } else {
  ...
  getPluginsForExtensionPoint(extensionPoint: string): LoadedPlugin[] {
    return Array.from(this.loadedPlugins.values())
      .filter(plugin => 
        plugin.isLoaded && 
        plugin.components[extensionPoint]
      );
  }
...
export const ExtensionPoint: React.FC<ExtensionPointProps> = ({
  ...
  const loadExtensionComponents = async () => {
    try {
      ...
      const { fetchInstalledPlugins, filterByExtensionPoint, filterEnabledPlugins } = await import('./plugin-integration');
      ...
      const relevantPlugins = installedPlugins.filter(p => 
        p.enabled && p.extensionPoints.includes(name)
      );
      ...
      for (const pluginMeta of relevantPlugins) {
        try {
          console.log(`🎯 Loading plugin "${pluginMeta.id}" for extension point "${name}"`);
          const plugin = await pluginLoader.loadPlugin(pluginMeta);
          console.log(`🎯 Plugin loaded:`, plugin);
          
          if (plugin.isLoaded && plugin.components[name]) {
            console.log(`✅ Found component for extension point "${name}" in plugin "${pluginMeta.id}"`);
            loadedComponents.push({
              pluginId: plugin.metadata.id,
              component: plugin.components[name],
            });
          } else {
            console.log(`❌ No component found for extension point "${name}" in plugin "${pluginMeta.id}"`, {
      ...
      console.log(`🎯 Final loaded components for "${name}":`, loadedComponents);
